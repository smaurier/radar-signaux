...
describe('DeclarationService', () => {
  ...
  const chercher = (html: string) =>
    (
      service as unknown as {
        chercherDeclaration: (h: string) => string | null;
      }
    ).chercherDeclaration(html);

  it('reconnait le texte normalise standard "totalement conforme"', () => {
    expect(chercher('<p>Accessibilité : totalement conforme</p>')).toBe(
      'conforme',
    );
  });
  ...
  it('reconnait le texte normalise standard "partiellement conforme"', () => {
    expect(
      chercher('<footer>Accessibilité : partiellement conforme</footer>'),
    ).toBe('partiel');
  });
  ...
  it('ne trouve rien sur une page sans mention accessibilite (vrai negatif)', () => {
    expect(
      chercher(
        '<html><body>Bienvenue sur notre boutique en ligne</body></html>',
      ),
    ).toBeNull();
  });
  ...
  it('reconnait le gabarit officiel DINUM "en conformite totale avec le RGAA" (ajoutee 13/08)', () => {
    expect(
      chercher(
        'État de conformité : Exemple est en conformité totale avec le RGAA.',
      ),
    ).toBe('conforme');
  });
  ...
  it('reconnait le gabarit officiel DINUM "en conformite partielle avec le referentiel" (ajoutee 13/08)', () => {
    expect(
      chercher(
        'Exemple est en conformité partielle avec le référentiel général.',
      ),
    ).toBe('partiel');
  });

  it('reconnait la formulation negative "n\'est pas conforme au RGAA" (ajoutee 13/08)', () => {
    expect(chercher("Le site Exemple n'est pas conforme au RGAA.")).toBe(
      'non_conforme',
    );
  });

  it('reconnait la formulation narrative avec preposition "avec" (elargie 13/08)', () => {
    expect(chercher('Le site est non conforme avec le RGAA.')).toBe(
      'non_conforme',
    );
  });
});
