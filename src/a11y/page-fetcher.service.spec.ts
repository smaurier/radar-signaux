...
describe('PageFetcherService - decodage charset', () => {
  ...
  it('decode un buffer UTF-8 correctement sans aucun indice de charset (comportement par defaut inchange)', () => {
    const buffer = versArrayBuffer(
      Buffer.from('<p>mentions légales</p>', 'utf-8'),
    );
  ...
  it('decode un buffer ISO-8859-1 grace au charset du header Content-Type (le vrai bug corrige)', () => {
    ...
    const buffer = versArrayBuffer(
      Buffer.from('<p>mentions légales</p>', 'latin1'),
    );
    expect(decoder(buffer, 'text/html; charset=iso-8859-1')).toContain(
      'mentions légales',
    );
  });
  ...
  it('decode un buffer ISO-8859-1 via le <meta charset> quand le header Content-Type est absent', () => {
    const html =
      '<html><head><meta charset="iso-8859-1"></head><body>légales</body></html>';
  ...
  it('decode un buffer Windows-1252 (variante courante de ISO-8859-1) via le header', () => {
    const buffer = versArrayBuffer(
      Buffer.from('<p>société à responsabilité</p>', 'latin1'),
    );
    expect(decoder(buffer, 'text/html; charset=windows-1252')).toContain(
  ...
  it('replie sur UTF-8 sans planter si le charset annonce est invalide', () => {
    const buffer = versArrayBuffer(
      Buffer.from('<p>texte utf-8 normal</p>', 'utf-8'),
    );
    expect(() =>
      decoder(buffer, 'text/html; charset=ce-charset-nexiste-pas'),
    ).not.toThrow();
    expect(
      decoder(buffer, 'text/html; charset=ce-charset-nexiste-pas'),
    ).toContain('texte utf-8 normal');
  });
});
